...
이 프로젝트는 같은 소스에서 두 APK를 동시에 만듭니다.

- `monsterDebug`
  - 몬스터 로고와 문구가 있는 버전
  - applicationId: `com.example.touchmapper`
  - 앱 이름: `MONSTER Touch`
- `plainDebug`
  - 몬스터 로고와 몬스터 문구를 제거한 버전
  - applicationId: `com.example.touchmapper.plain`
  - 앱 이름: `Touch Mapper`

버전을 올릴 때는 반드시 두 variant를 함께 빌드하고, `outputs/`에 둘 다 복사해야 합니다.
...
outputs/Touch-Mapper-A-plain-debug.apk
```
...
버전 업을 하면 monsterDebug와 plainDebug 두 APK를 동시에 빌드하고 outputs 폴더에 둘 다 복사해줘.
```
